...
test("deve validar campo nome obrigatorio ", async ({ page }) => {
  await page.goto("http://localhost:3000/");
  //await page.click('//button[co( )="Aperte o play... se tiver coragem"]')
  ...
  await page.locator("input[name=email]").fill("test@esa.com");
...
test("deve validar campo email obrigatorio ", async ({ page }) => {
  await page.goto("http://localhost:3000/");
  //await page.click('//button[co( )="Aperte o play... se tiver coragem"]')
